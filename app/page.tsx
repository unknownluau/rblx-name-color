...
export default function Home() {
	...
	const [username, setUsername] = useState<string>('unknownIua');
	const [version, setVersion] = useState<number>(3);
	const colorRef = useRef<HTMLDivElement>(null);
	const brickColorRef = useRef<HTMLDivElement>(null);
	...
	useEffect(() => {
		...
		if (color) {
			...
			brickColorRef.current!.innerText = color.name;
		} else {
			colorRef.current!.style.backgroundColor = 'rgb(' + [0, 0, 0, 0].join(',') + ')';
			colorRef.current!.innerText = 'Impossible to display color';
			colorRef.current!.style.color = 'rgb(255, 0, 0, 1)';
			brickColorRef.current!.innerText = '';
		}
	}, [username, version]);
	...
	return (<>
		<div className="w-screen h-screen flex flex-col justify-center items-center">
			...
			<div ref={brickColorRef} className="mt-2 text-white text-lg">.</div>
			<div className="flex flex-grow"></div>
			<div className="my-5 text-slate-300">
				<Link href='https://github.com/unknownluau' className='text-blue-500 transition duration-200 hover:text-blue-400'>
					@unknownluau
				</Link>
				&nbsp;on GitHub |&nbsp;
				<Link href='https://devforum.roblox.com/u/unknownIua' className='text-blue-500 transition duration-200 hover:text-blue-400'>
					@unknownIua
				</Link>
				&nbsp;on Roblox &amp; DevForum
			</div>
